...
export default function PdfToWordTool() {
  ...
  async function handleFile(f) {
    if (!f) return;
    if (f.type !== 'application/pdf') { setError(t('invalid_file') || 'Please choose a PDF file.'); return; }
    ...
    try {
      const { blob, fileName } = await convertPdfToWord(f, (current, total) => {
        setPageCount(total);
      ...
      console.error(err); setError('Something went wrong, please try another file.');
        ...
        <div className="wrap">
          <div className="pdf-ws-card">
            ...
            <div className="pdf-ws-footer">
              ...
              {file && (
                <button type="button" className="btn btn-ghost pdf-ws-secondary" onClick={reset}>
                  {t('choose_another') || 'Choose another file'}
                </button>
              )}
