...
import { jpgToPdfDicts } from "../i18n";
import { jpgToPdf, formatSize } from "../logic/jpgToPdf";

export default function JpgToPdfTool() {
  const t = useToolI18n(jpgToPdfDicts);
  ...
  function addFiles(list) {
    const images = Array.from(list).filter((f) => /^image\/(jpeg|png)$/.test(f.type));
  ...
  async function handleConvert() {
    ...
    try {
      const { blob, fileName } = await jpgToPdf(files, (current, total) => {
          ...
          <div className="pdf-ws-card">
            <div className="pdf-ws-grid">
              <div className="pdf-ws-left">
                ...
                <input ref={fileInputRef} type="file" accept="image/jpeg,image/png" multiple style={{ display: "none" }}
