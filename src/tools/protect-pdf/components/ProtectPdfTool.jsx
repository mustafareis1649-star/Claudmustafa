...
import { protectPdfDicts } from '../i18n';
import { protectPdf, unlockPdf, formatSize } from '../logic/protectPdf';

export default function ProtectPdfTool() {
  const t = useToolI18n(protectPdfDicts);
  ...
  async function handleRun() {
    if (!password) { setError(t('need_password')); return; }
    setBusy(true); setError('');
    try {
      const { blob, fileName } = mode === 'protect'
        ? await protectPdf(file, password)
        : await unlockPdf(file, password);
      setResult({ blobUrl: URL.createObjectURL(blob), fileName });
    } catch (err) {
      console.error(err);
      setError(mode === 'unlock' ? t('wrong_password') : t('generic_error'));
  ...
  function reset(nextMode) {
    ...
    setFile(null); setResult(null); setError(''); setPassword(''); setPageCount(null);
          ...
          <div className="pdf-ws-card">
            <div className="pdf-ws-grid">
              <div className="pdf-ws-left">
                ...
                <div className="pdf-ws-btn-group" style={{ marginBottom: 4 }}>
                  <button type="button" className={mode === 'protect' ? 'btn btn-ink' : 'btn btn-ghost'}
                    onClick={() => reset('protect')}>{t('tab_protect')}</button>
                  <button type="button" className={mode === 'unlock' ? 'btn btn-ink' : 'btn btn-ghost'}
                    onClick={() => reset('unlock')}>{t('tab_unlock')}</button>
                </div>
                ...
                {file && !result && (
                  <div className="pdf-ws-controls">
                    <input type="password" className="pdf-ws-ctrl-input" value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      placeholder={mode === 'protect' ? t('new_password_placeholder') : t('current_password_placeholder')} />
                  </div>
                )}
              </div>
            ...
            <div className="pdf-ws-footer">
              {result ? (
                ...
                <button className="btn btn-signal pdf-ws-primary" onClick={handleRun} disabled={busy}>
                  {busy ? t('working') : (mode === 'protect' ? t('protect_btn') : t('unlock_btn'))}
                </button>
              ) : null}
              {file && <button type="button" className="btn btn-ghost pdf-ws-secondary" onClick={() => reset()}>{t('choose_another')}</button>}
              <p className="footnote">{t(mode === 'protect' ? 'footnote_protect' : 'footnote_unlock')}</p>
            </div>
          </div>
        </div>
      </div>
