...
import { wordToPdfDicts } from '../i18n';
import { convertWordToPdf, formatSize } from '../logic/wordToPdf';

const DOCX_TYPE = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';

export default function WordToPdfTool() {
  const t = useToolI18n(wordToPdfDicts);
  ...
  function isDocx(f) {
    return f.type === DOCX_TYPE || /\.docx$/i.test(f.name);
  }

  async function handleFile(f) {
    if (!f) return;
    if (!isDocx(f)) { setError(t('invalid_file')); return; }
    setError(''); setFile(f); setResult(null); setBusy(true); setStatus(t('converting'));
    try {
      const { blob, fileName } = await convertWordToPdf(f);
              ...
              <div className="pdf-ws-left">
                ...
                {!file ? (
                  ...
                  <div className="pdf-ws-file-row">
                    <div className="pdf-ws-file-ic">DOC</div>
                ...
                <input ref={fileInputRef} type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
