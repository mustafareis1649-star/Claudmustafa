...
import { splitPdfDicts } from '../i18n';
import { splitPdf, formatSize } from '../logic/splitPdf';

export default function SplitPdfTool() {
  const t = useToolI18n(splitPdfDicts);
  ...
  async function handleFile(f) {
    ...
    try {
      const { blob, fileName, pageCount: pc } = await splitPdf(f, (current, total) => {
        setPageCount(total);
        setProgress(Math.round((current / total) * 100));
        setStatus(t('status_splitting').replace('{current}', current).replace('{total}', total));
      });
      setStatus(t('status_done').replace('{count}', pc));
