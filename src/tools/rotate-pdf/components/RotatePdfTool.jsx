...
import { rotatePdfDicts } from '../i18n';
import { rotatePdf, formatSize } from '../logic/rotatePdf';

export default function RotatePdfTool() {
  const t = useToolI18n(rotatePdfDicts);
          ...
          <div className="pdf-ws-card">
                ...
                {file && !result && (
                  <div className="pdf-ws-controls">
                    <div className="pdf-ws-btn-group">
                      {[{ v: 90, label: '↻ 90°' }, { v: 180, label: '↻ 180°' }, { v: 270, label: '↺ 90°' }].map((opt) => (
                        <button key={opt.v} type="button" className={degrees === opt.v ? 'btn btn-ink' : 'btn btn-ghost'}
                          onClick={() => setDegrees(opt.v)}>{opt.label}</button>
                      ))}
                    </div>
                  </div>
                )}
            ...
            <div className="pdf-ws-footer">
              {result ? (
                ...
                <button className="btn btn-signal pdf-ws-primary" onClick={handleRotate} disabled={busy}>{busy ? t('rotating') : t('rotate_btn')}</button>
              ) : null}
