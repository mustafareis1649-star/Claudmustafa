...
import { pdfMetadataEditorDicts } from "../i18n";
import { readMetadata, writeMetadata, formatSize } from "../logic/pdfMetadataEditor";

export default function PdfMetadataEditorTool() {
  const t = useToolI18n(pdfMetadataEditorDicts);
  ...
  async function pickFile(f) {
    ...
    try {
      const existing = await readMetadata(f);
      setMeta({ title: existing.title || "", author: existing.author || "", subject: existing.subject || "",
        keywords: existing.keywords || "", creator: existing.creator || "" });
    } catch { /* ignore */ }
  }
  ...
  function reset() {
    ...
    setFile(null); setResult(null); setError(""); setStatus(""); setMeta({ title: "", author: "", subject: "", keywords: "", creator: "" }); setPageCount(null);
  }
          ...
          <div className="pdf-ws-card">
              ...
              <div className="pdf-ws-left">
                ...
                {file && (
                  <div className="pdf-ws-controls">
                    {["title", "author", "subject", "keywords", "creator"].map((key) => (
                      <div key={key}>
                        <label className="pdf-ws-ctrl-label">{t(`meta_${key}`)}</label>
                        <input type="text" className="pdf-ws-ctrl-input" value={meta[key]}
                          onChange={(e) => setMeta((prev) => ({ ...prev, [key]: e.target.value }))}
                          placeholder={t(`meta_${key}_placeholder`)} />
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="pdf-ws-footer">
              {result ? (
                ...
                <button className="btn btn-signal pdf-ws-primary" onClick={handleSave} disabled={busy}>{busy ? t("saving") : t("save_btn")}</button>
              ) : null}
